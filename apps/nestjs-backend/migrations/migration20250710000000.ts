...
export class Migration20250710000000 extends Migration {
  async up(): Promise<void> {
    ...
    this.addSql(
      'create table "two_factor_auth" ("id" uuid not null default gen_random_uuid(), "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "code" varchar(255) not null, "user_id" uuid not null, constraint "two_factor_auth_pkey" primary key ("id"));',
    );

    // Create revoked_refresh_token table
    this.addSql(
      'create table "revoked_refresh_token" ("id" uuid not null default gen_random_uuid(), "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "refresh_token" varchar(1000) not null, constraint "revoked_refresh_token_pkey" primary key ("id"));',
    );

    // Add foreign key constraints
    this.addSql(
      'alter table "two_factor_auth" add constraint "two_factor_auth_user_id_foreign" foreign key ("user_id") references "user" ("id") on update cascade on delete cascade;',
    );
  }
