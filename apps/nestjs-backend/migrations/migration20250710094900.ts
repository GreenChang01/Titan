...
export class Migration20250710094900 extends Migration {
	async up(): Promise<void> {
		...
		this.addSql('create table "project_material" ("id" uuid not null default gen_random_uuid(), "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "aliyun_file_id" varchar(255) not null, "file_name" varchar(255) not null, "file_path" varchar(1000) not null, "file_type" varchar(100) not null, "file_size" bigint null, "thumbnail_url" varchar(500) null, "metadata" jsonb null, "project_id" uuid not null, constraint "project_material_pkey" primary key ("id"));');
		this.addSql('create unique index "project_material_project_id_aliyun_file_id_unique" on "project_material" ("project_id", "aliyun_file_id");');

		// Add foreign key constraints
		this.addSql('alter table "aliyun_drive_config" add constraint "aliyun_drive_config_user_id_foreign" foreign key ("user_id") references "user" ("id") on update cascade on delete cascade;');
		this.addSql('alter table "project" add constraint "project_user_id_foreign" foreign key ("user_id") references "user" ("id") on update cascade on delete cascade;');
		this.addSql('alter table "project_material" add constraint "project_material_project_id_foreign" foreign key ("project_id") references "project" ("id") on update cascade on delete cascade;');
	}
