...
export class AliyunDriveController {
  ...
  async createConfig(@User() user: UserEntity, @Body() createDto: CreateAliyunDriveConfigDto) {
  ...
  @Get('config')
  async getConfig(@User() user: UserEntity) {
  ...
  @Put('config/:id')
  async updateConfig(@Param('id') id: string, @User() user: UserEntity, @Body() updateDto: UpdateAliyunDriveConfigDto) {
    const config = await this.aliyunDriveService.findByUser(user);
  ...
  async deleteConfig(@Param('id') id: string, @User() user: UserEntity) {
    const config = await this.aliyunDriveService.findByUser(user);
  ...
  // WebDAV 文件操作端点
  @Get('files')
  async listFiles(@User() user: UserEntity, @Query() listDto: ListFilesDto) {
    const config = await this.aliyunDriveService.findByUser(user);
  ...
  @Get('files/download')
  async downloadFile(@User() user: UserEntity, @Query() downloadDto: DownloadFileDto, @Res() res: Response) {
    const config = await this.aliyunDriveService.findByUser(user);
  ...
  @Post('directories')
  async createDirectory(@User() user: UserEntity, @Body() createDirDto: CreateDirectoryDto) {
    const config = await this.aliyunDriveService.findByUser(user);
  ...
  @Delete('items')
  async deleteItems(@User() user: UserEntity, @Body() deleteDto: DeleteItemsDto) {
    const config = await this.aliyunDriveService.findByUser(user);
  ...
  @Post('items/move')
  async moveItem(@User() user: UserEntity, @Body() moveDto: MoveItemDto) {
    const config = await this.aliyunDriveService.findByUser(user);
  ...
  @Post('items/copy')
  async copyItem(@User() user: UserEntity, @Body() copyDto: CopyItemDto) {
    const config = await this.aliyunDriveService.findByUser(user);
  ...
  async syncConfig(@Param('id') id: string, @User() user: UserEntity) {
    const config = await this.aliyunDriveService.findByUser(user);
