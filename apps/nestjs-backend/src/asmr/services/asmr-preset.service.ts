...
@Injectable()
export class ASMRPresetService {
  ...
  async incrementUsageCount(id: string): Promise<void> {
    ...
    if (preset) {
      preset.usageCount += 1;
      await this.presetRepository.persistAndFlush(preset);
    }
  }
  ...
  private async initializeDefaultPresets(): Promise<void> {
    const existingPresets = await this.presetRepository.count();
    ...
    for (const preset of defaultPresets) {
      ...
      await this.presetRepository.persistAndFlush(entity);
    }
