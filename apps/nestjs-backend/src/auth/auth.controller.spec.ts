...
import {mock, mockDeep, type DeepMockProxy} from 'jest-mock-extended';
import {type LoginCredentialsBodyDto, type LoginTwoFactorAuthBodyDto} from '@titan/shared';
...
describe('AuthController', () => {
	...
	describe('environment configuration', () => {
		it('should set secure cookies in production environment', async () => {
			...
			const productionConfigService = mockDeep<ConfigService>();
			productionConfigService.get.mockImplementation((key: ConfigKey) => {
				if (key === ConfigKey.NODE_ENV) {
					return 'production';
				}
