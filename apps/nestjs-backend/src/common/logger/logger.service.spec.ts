...
describe('Logger', () => {
	...
	beforeEach(async () => {
		...
		logger = await moduleReference.resolve<Logger>(Logger);
	});
	...
	it('should be a transient-scoped instance', async () => {
		...
		const logger1 = await moduleReference.resolve<Logger>(Logger);
		const logger2 = await moduleReference.resolve<Logger>(Logger);
