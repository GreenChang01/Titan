...
export class MediaProcessingService {
  ...
  private async generateASMRVideo(
    ...
    return new Promise((resolve, reject) => {
      ...
      command
        ...
        .on('error', (err) => {
          this.logger.error('ASMR video generation failed:', err.message);
          reject(err);
        })
        .run();
    });
  ...
  private async generateDynamicBackgroundVideo(
    ...
    return new Promise((resolve, reject) => {
      ...
      command
        ...
        .on('error', (err) => {
          this.logger.error('Dynamic background video generation failed:', err.message);
          reject(err);
        })
        .run();
    });
  ...
  private getAssetBySlot(
    ...
    return assets.get(assetMapping.assetId) || null;
  }
  ...
  async combineAudioTracks(audioFiles: string[], outputPath: string): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      command
        ...
        .on('error', (err) => {
          this.logger.error('Audio combination failed:', err.message);
          reject(err);
        })
        .run();
    });
