          ...
          >
            <CardContent className="p-4 text-center">
              <div className="mb-2 flex justify-center">
                {getFileTypeIcon(file, "h-8 w-8 text-muted-foreground")}
              </div>
      ...
      {files.map((file) => (
        ...
        >
          <Checkbox
            ...
            onCheckedChange={(checked) => { onFileSelect(file.path, checked as boolean); }}
...
export function AliyunDriveBrowser({ 
  ...
  const { data: fileData, isLoading, error, refetch } = useAliyunDriveFiles(params);
      ...
      <CardContent className="space-y-4">
        ...
        {selectedFiles.length > 0 && (
          <div className="flex items-center gap-2 p-2 bg-accent rounded-md">
            ...
            {onFilesSelected ? <Button size="sm" onClick={() => { onFilesSelected(selectedFileObjects); }}>
              添加到项目
            </Button> : null}
