	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent className="sm:max-w-md">
				<DialogHeader>
					<DialogTitle>{isEditing ? '编辑阿里云盘配置' : '添加阿里云盘配置'}</DialogTitle>
					<DialogDescription>配置WebDAV连接信息以访问您的阿里云盘</DialogDescription>
				</DialogHeader>
		...
		<Card>
			<CardHeader className="flex flex-row items-center justify-between pb-2">
				<div className="flex items-center gap-2">
					...
					<Badge variant={config.isActive ? 'default' : 'secondary'}>{config.isActive ? '活跃' : '停用'}</Badge>
				</div>
						...
						<AlertDialog>
							...
							<AlertDialogContent>
								<AlertDialogHeader>
									...
									<AlertDialogDescription>您确定要删除配置 "{config.name}" 吗？此操作无法撤销。</AlertDialogDescription>
								</AlertDialogHeader>
								<AlertDialogFooter>
