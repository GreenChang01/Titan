...
export function JobProgressMonitor({jobId, onJobComplete, className}: JobProgressMonitorProps): JSX.Element {
  const {data: progress, error, refetch} = useAsmrJobProgress(jobId, {
    onJobComplete, // Pass the callback directly to the hook
  });
  const retryMutation = useRetryAsmrJob({
    onSuccess() {
      refetch(); // Refresh progress after retry
    },
  });
      ...
      <CardContent className="space-y-4">
        ...
        {(progress.status === 'processing' || progress.status === 'queued') && (
          <div className="space-y-2">
            ...
            {progress.currentStep ? <p className="text-xs text-muted-foreground">当前步骤: {progress.currentStep}</p> : null}
            {progress.estimatedTimeRemaining ? <p className="text-xs text-muted-foreground">
        ...
        {/* Error Message */}
        {progress.status === 'failed' && progress.error ? <div className="p-3 bg-red-50 border border-red-200 rounded-md">
