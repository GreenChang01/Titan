...
import {asmrApi, type JobListParams, type JobListResponse, type CostEstimate, type ServiceStatus, type PresetsResponse} from '../api/asmr.api';
import {queryKeys, invalidateQueries} from '@/lib/react-query';
import {type ApiError} from '@/lib/api/client';

// Job list hook (simplest one to start migration)
export function useAsmrJobs(params?: JobListParams) {
  return useQuery({
    queryKey: queryKeys.asmr.jobs(params || {}),
    queryFn: async () => asmrApi.listJobs(params),
