...
const aiPromptApi = {
  async getPrompts(filters: AIPromptFilters = {}): Promise<AIPrompt[]> {
    ...
    if (filters.isPublic !== undefined) searchParams.append('isPublic', String(filters.isPublic));
...
export function useAIPrompts(filters: AIPromptFilters = {}) {
  return useQuery({
    ...
    queryFn: () => aiPromptApi.getPrompts(filters),
...
export function useAIPrompt(id: string) {
  return useQuery({
    ...
    queryFn: () => aiPromptApi.getPrompt(id),
    enabled: !!id,
  });
}
...
export function useUpdateAIPrompt() {
  ...
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: UpdateAIPromptRequest }) =>
  ...
  return useMutation({
    ...
    onMutate: async (id) => {
      ...
      activeQueries.forEach(query => {
        ...
        const currentData = queryClient.getQueryData(queryKey) as AIPrompt[] | undefined;
