...
export const aliyunDriveApi = {
  ...
  // File operations
  async getFiles(params?: FileListParams): Promise<DirectoryContent> {
    ...
    return apiClient.get<DirectoryContent>('/aliyun-drive/files', queryParams);
  },
  ...
  async downloadFile(path: string): Promise<Blob> {
    const response = await fetch(
      `${apiClient.baseURL}/aliyun-drive/files/download?path=${encodeURIComponent(path)}`,
      {
        method: 'GET',
        headers: {
  ...
  async getFileUrl(path: string, expiresIn?: number): Promise<{url: string; expiresAt: string}> {
    ...
    return apiClient.get('/aliyun-drive/files/url', params);
  },
  ...
  async searchFiles(
    ...
    return apiClient.get<DriveFile[]>('/aliyun-drive/files/search', params);
  },
  ...
  async getDriveStats(configId?: string): Promise<DriveStats> {
    ...
    return apiClient.get<DriveStats>('/aliyun-drive/stats', params);
  },
  ...
  async syncDrive(configId?: string): Promise<{success: boolean; message: string; syncedCount: number}> {
    ...
    return apiClient.post('/aliyun-drive/sync', params);
  },
  ...
  async getThumbnail(path: string, size?: 'small' | 'medium' | 'large'): Promise<Blob> {
    ...
    const response = await fetch(
      `${apiClient.baseURL}/aliyun-drive/files/thumbnail?${new URLSearchParams(params)}`,
      {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${apiClient.token}`,
